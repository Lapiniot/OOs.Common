...
[TestClass]
public class AsyncSemaphoreLightConstructorShould
{
    [TestMethod]
    public void ThrowArgumentOutOfRange_GivenNegativeInitialCount() =>
        Assert.ThrowsExactly<ArgumentOutOfRangeException>(() => new ASL(-1));

    [TestMethod]
    public void ThrowArgumentOutOfRange_GivenNegativeMaxCount() =>
        Assert.ThrowsExactly<ArgumentOutOfRangeException>(() => new ASL(0, -1));

    [TestMethod]
    public void ThrowArgumentOutOfRange_GivenZeroMaxCount() =>
        Assert.ThrowsExactly<ArgumentOutOfRangeException>(() => new ASL(0, 0));

    [TestMethod]
    public void ThrowArgumentOutOfRange_GivenInitialCountGraterThanMaxCount() =>
        Assert.ThrowsExactly<ArgumentOutOfRangeException>(() => new ASL(2, 1));
