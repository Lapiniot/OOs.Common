...
public class ConstructorShould
{
    ...
    public void ThrowArgumentOutOfRange_GivenNegativeInitialCount() =>
        Assert.ThrowsExactly<ArgumentOutOfRangeException>(() => new Threading.AsyncSemaphore(-1));

    [TestMethod]
    public void ThrowArgumentOutOfRange_GivenNegativeMaxCount() =>
        Assert.ThrowsExactly<ArgumentOutOfRangeException>(() => new Threading.AsyncSemaphore(0, -1));

    [TestMethod]
    public void ThrowArgumentOutOfRange_GivenZeroMaxCount() =>
        Assert.ThrowsExactly<ArgumentOutOfRangeException>(() => new Threading.AsyncSemaphore(0, 0));

    [TestMethod]
    public void ThrowArgumentOutOfRange_GivenInitialCountGraterThanMaxCount() =>
        Assert.ThrowsExactly<ArgumentOutOfRangeException>(() => new Threading.AsyncSemaphore(2, 1));
